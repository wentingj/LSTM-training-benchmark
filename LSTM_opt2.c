...
void  LSTM_backward(int batch_size, int time_step, int input_dim, int hid, 
    ...
    //        dwoh[i] += dwoh_allt[index];
    //        
    //        dwfh[i+1] += dwfh_allt[index+1];
    //        dwih[i+1] += dwih_allt[index+1];
    //        dwch[i+1] += dwch_allt[index+1];
    //        dwoh[i+1] += dwoh_allt[index+1];
    //        
    //        dwfh[i+2] += dwfh_allt[index+2];
    //        dwih[i+2] += dwih_allt[index+2];
    //        dwch[i+2] += dwch_allt[index+2];
    //        dwoh[i+2] += dwoh_allt[index+2];
    //        
    //        dwfh[i+3] += dwfh_allt[index+3];
    //        dwih[i+3] += dwih_allt[index+3];
    //        dwch[i+3] += dwch_allt[index+3];
    //        dwoh[i+3] += dwoh_allt[index+3];
    //        
    //        dwfh[i+4] += dwfh_allt[index+4];
    //        dwih[i+4] += dwih_allt[index+4];
    //        dwch[i+4] += dwch_allt[index+4];
    //        dwoh[i+4] += dwoh_allt[index+4];
    //    }
    //}
    //#pragma omp parallel for private(i) 
    //for(j = 0; j < time_step; j++) {
